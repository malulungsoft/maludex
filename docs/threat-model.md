...
| Desktop workspace sync exposes project paths | By default the bridge writes mobile-created or mobile-selected project paths into Codex Desktop's local workspace-root state so the desktop app can show those projects. This does not write prompt bodies and can be disabled with `BRIDGE_SYNC_CODEX_DESKTOP=0` or `--no-desktop-sync`. |
