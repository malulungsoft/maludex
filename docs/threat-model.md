...
| Desktop workspace sync exposes project paths and thread ids | By default the bridge writes mobile-created or mobile-selected project paths into Codex Desktop's local workspace-root state and mobile-started thread ids into `session_index.jsonl` so the desktop app can show those projects and chats. This does not write prompt bodies into the desktop index and can be disabled with `BRIDGE_SYNC_CODEX_DESKTOP=0` or `--no-desktop-sync`. |
