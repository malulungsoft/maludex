...
- Registers mobile-created and mobile-selected `cwd` paths in Codex Desktop's local workspace-root state so the desktop app can surface those projects and their existing Codex threads. This can be disabled with `BRIDGE_SYNC_CODEX_DESKTOP=0` or `--no-desktop-sync`.
