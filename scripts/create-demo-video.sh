...
names=(live-chat prompt-queue approval-card desktop-sync completion composer)
offsets=(2 14 32 38 50 56)
for index in "${!names[@]}"; do
  ffmpeg -y \
    -ss "${offsets[$index]}" \
    -i "$OUT_VIDEO" \
    -frames:v 1 \
    "$SCREENSHOT_DIR/${names[$index]}.png" >/tmp/maludex-demo-screenshot.log 2>&1
done
